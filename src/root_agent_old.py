...
our multi-agent educational system following the proper ADK LlmAgent pattern.
"""
...
try:
    ...
    ADK_AVAILABLE = True
    logger.info("✅ ADK components successfully imported")
...
Use the process_educational_query tool to access the specialized agents and provide accurate, subject-specific responses.
"""

def process_educational_query(query: str, subject_hint: str = "", difficulty: str = "MIDDLE") -> str:
    """
    Process an educational query through the multi-agent system.
    
    Args:
        query: The student's question or request
        subject_hint: Optional hint about the subject (math, science, music, or interdisciplinary)
        difficulty: The difficulty level (ELEMENTARY, MIDDLE, HIGH, COLLEGE)
    
    Returns:
        Educational response from the appropriate specialized agent(s)
    """
    try:
        # Create our internal multi-agent system
        ...
        # Determine subject from hint or infer from query
        subject_enum = None
        if subject_hint:
            try:
                subject_enum = SubjectType(subject_hint.upper())
            except ValueError:
                pass
        
        if not subject_enum:
            subject_enum = _infer_subject_from_message(query)
        ...
        # Process synchronously for tool usage
        import asyncio
        try:
            ...
            if loop.is_running():
                ...
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    future = executor.submit(_process_request_sync, session_manager, coordinator, request, subject_enum)
                    response = future.result(timeout=30)
            else:
                response = loop.run_until_complete(_process_request_async(session_manager, coordinator, request, subject_enum))
        except RuntimeError:
            # No event loop exists, create one
            response = asyncio.run(_process_request_async(session_manager, coordinator, request, subject_enum))
        
        return response
        
    except Exception as e:
        logger.error(f"Error in process_educational_query: {e}")
        return f"🚨 **Educational System Error**\n\nI apologize, but I encountered an error while processing your educational query:\n\n`{str(e)}`\n\nPlease try rephrasing your question or try again in a moment."

def _process_request_sync(session_manager, coordinator, request, subject_enum):
    """Synchronous wrapper for async request processing"""
    return asyncio.run(_process_request_async(session_manager, coordinator, request, subject_enum))

async def _process_request_async(session_manager, coordinator, request, subject_enum):
    ...
    if subject_enum:
        ...
        logger.info("🔄 Interdisciplinary question detected - consulting all specialized agents")
        involving_subjects = [SubjectType.MATH, SubjectType.SCIENCE, SubjectType.MUSIC]
        response = await coordinator.process_complex_query(request, involving_subjects)
        
        return f"🎓 **{response.agent_name}** (Confidence: {response.confidence_score:.1%})\n\n{response.response_text}\n\n---\n*Multi-Agent Collaborative Response (Math 🔢 + Science 🔬 + Music 🎵)*"

def _infer_subject_from_message(message: str) -> Optional[SubjectType]:
    """Infer the subject from the message content."""
    message_lower = message.lower()
